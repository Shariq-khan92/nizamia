...
export const CostingDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCostings = async () => {
      try {
        const data = await api.getCostings();
        // Map API data to CostingRecord interface
        const mappedRecords: CostingRecord[] = data.map((item: any) => ({
          costingId: item.costingId,
          buyerName: item.meta?.customer || 'Unknown',
          styleNumber: item.meta?.styleNo || 'Unknown',
          revision: 1, // Default to 1 as revision tracking isn't implemented yet
          finalPrice: item.finalFobPkr || 0,
          marginPct: item.marginPct || 0,
          dateCreated: item.meta?.date || new Date(item.createdAt).toISOString().split('T')[0],
          status: item.status
        }));
        setCostingRecords(mappedRecords);
      } catch (error) {
