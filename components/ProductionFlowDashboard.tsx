                                ...
                                return (
                                    <tr key={job.id} className="group hover:bg-gray-50">
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-[10px] text-gray-500">{getBuyerName(job.styles[0]?.buyer)}</div>
                                        </td>
                ...
                job.dailyLogs?.forEach(log => {
                    ...
                    if (logDate >= startStr && (department === 'All' || log.stage === department)) {
                        rows.push([
                            ...
                            getBuyerName(job.styles[0]?.buyer) || '-',
                            log.stage,
                            log.lineId || '-',
...
export const ProductionFlowDashboard: React.FC<ProductionFlowDashboardProps> = ({ jobs, onUpdateJob }) => {
    ...
    // Load production lines from settings API
    useEffect(() => {
        api.getProductionLines()
            .then((lines: any[]) => setProductionLines(lines.map((l) => l.name)))
            .catch(console.error);
    }, []);
                                        ...
                                        .map(job => {
                                            ...
                                            return (
                                                <tr key={job.id} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 text-gray-600">{getBuyerName(job.styles[0]?.buyer)}</td>
