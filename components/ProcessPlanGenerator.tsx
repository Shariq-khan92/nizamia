...
export const ProcessPlanGenerator: React.FC<ProcessPlanGeneratorProps> = ({ job, onClose, onIssue }) => {
  ...
  // Initialize steps based on job requirements from API
  useEffect(() => {
      api.getProcessSteps()
        .then((apiSteps: any[]) => {
            // Sort by sequenceOrder if present
            const sorted = [...apiSteps].sort((a, b) => (a.sequenceOrder || 0) - (b.sequenceOrder || 0));

            // Map to local shape
            let mapped: ProcessStep[] = sorted.map((s) => ({
                id: s.id,
                stage: s.name,
                type: 'Internal',
                vendor: '',
            ...
            // If stages exist in DB but should be skipped, filter them out
            if (!hasEmb) {
                mapped = mapped.filter(step => step.stage.toLowerCase() !== 'embellishment');
            }
            if (!hasWash) {
                mapped = mapped.filter(step => step.stage.toLowerCase() !== 'washing');
            }
