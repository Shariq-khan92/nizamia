...
import {
    ...
    Save, CircleDashed, GitMerge, RefreshCw
} from 'lucide-react';
...
export const JobManagerDashboard: React.FC<JobManagerDashboardProps> = ({
    ...
    const handleCreateJob = async () => {
        ...
        if (editingJobId) {
            if (onUpdateJob) {
                try {
                    const updated = await onUpdateJob(editingJobId, newJob);
                    // Update local list to reflect immediately if needed, though parent should handle via props
                    onUpdateJobs(jobs.map(j => j.id === editingJobId ? updated : j));
                } catch (e) {
                    console.error(e);
            ...
            if (onCreateJob) {
                try {
                    const saved = await onCreateJob(newJob);
                    onUpdateJobs([saved, ...jobs]);
                } catch (e) {
                    console.error(e);
    ...
    const confirmDeleteJob = async () => {
        if (deletePassword === 'admin') {
            if (onDeleteJob && jobToDeleteId) {
                try {
                    await onDeleteJob(jobToDeleteId);
    ...
    const confirmDeletePlan = async () => {
        ...
        if (planDeletePassword === 'admin') {
            ...
            if (onUpdateJob) {
                try {
                    await onUpdateJob(planningJob.id, updatedJob);
                    onUpdateJobs(jobs.map(j => j.id === planningJob.id ? updatedJob : j));
                } catch (e) {
                    console.error(e);
                    alert("Failed to update job plans");
                    return;
                }
            } else {
                onUpdateJobs(jobs.map(j => j.id === planningJob.id ? updatedJob : j));
    ...
    const updateJobState = async (updated: JobBatch) => {
        if (onUpdateJob) {
            try {
                await onUpdateJob(updated.id, updated);
        ...
        if (planningJob && planningJob.id === updated.id) {
            setPlanningJob(updated);
        }
    };

    const handleIssueFabric = async (reqs: PurchasingRequest[]) => {
        ...
        await updateJobState(updatedJob);
        setActivePlanGenerator(null);
    };

    const handleIssueTrims = async (reqs: PurchasingRequest[]) => {
        if (!planningJob) return;
        const updatedJob = {
            ...planningJob,
    ...
    const handleIssueEmbellishment = async (reqs: WorkOrderRequest[]) => {
        if (!planningJob) return;
        const updatedJob = {
            ...planningJob,
    ...
    const handleIssueCutting = async (details: CuttingPlanDetail[]) => {
        if (!planningJob) return;
        const updatedJob = {
            ...planningJob,
    ...
    const handleIssueFinishing = async (updated: JobBatch) => {
        await updateJobState(updated);
        setActivePlanGenerator(null);
    };

    const handleIssueSampling = async (updated: JobBatch) => {
        await updateJobState(updated);
        setActivePlanGenerator(null);
    };

    const handleIssueTesting = async () => {
        if (!planningJob) return;
    ...
    const handleIssueProcess = async (schedules: Record<string, { startDate: string; endDate: string }>) => {
        if (!planningJob) return;
        const updatedJob = {
            ...planningJob,
    ...
    const handleRefreshJob = async () => {
        ...
        try {
            // @ts-ignore
            const updated = await jobService.refresh(planningJob.id);
            setPlanningJob(updated);
            if (onUpdateJob) {
                onUpdateJobs(jobs.map(j => j.id === planningJob.id ? updated : j));
            }
            setSuccessToast('Job data refreshed from Order Management.');
        } catch (error) {
            console.error(error);
            alert('Failed to refresh job data.');
        } finally {
    ...
    const handleSavePPNotes = async () => {
        ...
        const updatedJob = { ...targetJob, styles: updatedStyles };
    ...
    return (
        <div className="flex flex-col h-full space-y-6 animate-in fade-in duration-500">
                        ...
                        <div className="px-10 py-6 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="p-2 text-gray-400 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-tight"
                                    title="Sync data with Order Management"
                                >
                                    <RefreshCw size={18} className={refreshingJob ? "animate-spin" : ""} />
                                    {refreshingJob ? 'Syncing...' : 'Refresh Data'}
                                </button>
                                <button
                                            ...
                                            <tbody className="divide-y divide-gray-100 bg-white">
                                                {availableOrders.filter(o => o.status !== 'Draft' && !jobs.some(j => j.styles.some(s => s.id === o.id))).map(order => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-3 font-bold text-gray-500 uppercase text-[10px]">{getBuyerName(order.buyer)}</td>
            ...
            {/* Added Stitching and Washing */}
            {
                activePlanGenerator === 'Stitching' && planningJob && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center">
                            <h2 className="text-xl font-bold mb-2">Stitching Plan</h2>
                            <p className="text-gray-500 mb-4">Stitching plan module is currently under development.</p>
                            <button onClick={() => setActivePlanGenerator(null)} className="px-4 py-2 bg-gray-200 rounded text-sm font-bold">Close</button>
                        </div>
                    </div>
                )
            }
            {
                activePlanGenerator === 'Washing' && planningJob && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center">
                            <h2 className="text-xl font-bold mb-2">Washing Plan</h2>
                            <p className="text-gray-500 mb-4">Washing plan module is currently under development.</p>
                            <button onClick={() => setActivePlanGenerator(null)} className="px-4 py-2 bg-gray-200 rounded text-sm font-bold">Close</button>
                        </div>
                    </div>
                )
