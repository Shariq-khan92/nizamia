...
interface PlanningDashboardProps {
    ...
    parcels: Parcel[];
    onCreateParcel?: (parcel: Parcel) => Promise<Parcel | void>;
    onUpdateParcel?: (parcel: Parcel) => Promise<Parcel | void>;
...
export const PlanningDashboard: React.FC<PlanningDashboardProps> = ({
    ...
    parcels, onCreateParcel, onUpdateParcel, availableBuyers, companyDetails,
    ...
    const handleMarkReceived = () => {
        ...
        selectedParcelIds.forEach(async id => {
            const p = parcels.find(parcel => parcel.id === id);
            if (p && onUpdateParcel) {
                const updated = { ...p, status: 'Received' as const, receivedDate: new Date().toISOString().split('T')[0] };
    ...
    const handleConfirmParcel = async () => {
        ...
        // onUpdateParcels([newParcel, ...parcels]); - Replaced by API call
        if (onCreateParcel) {
            await onCreateParcel(newParcel);
        }
    ...
    const handleUpdateTracking = async () => {
        ...
        const parcelToUpdate = updatedParcels.find(p => p.id === parcelId);
        if (parcelToUpdate && onUpdateParcel) {
            await onUpdateParcel(parcelToUpdate);
        }
