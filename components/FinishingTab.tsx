...
export const FinishingTab: React.FC<FinishingTabProps> = ({ data, onUpdate, sizeGroups = [] }) => {
  const [isEditing, setIsEditing] = useState(false);
    ...
    const [baseInstruction, setBaseInstruction] = useState<PackingInstruction>(INITIAL_INSTRUCTION);
    const [instructionTemplates, setInstructionTemplates] = useState<PackingInstruction[]>([]);
  ...
  const availableMatrix = useMemo((): Record<string, number> => {
    ...
    // Load default packing instruction template from API
    useEffect(() => {
        api.getPackingInstructions()
            .then((list: any[]) => {
                setInstructionTemplates(list || []);
                const defaultTemplate = (list || []).find((t: any) => t.isDefault) || (list || [])[0];
                if (defaultTemplate) {
                    const template: PackingInstruction = {
                        ...INITIAL_INSTRUCTION,
                        ...defaultTemplate,
                        id: '',
                        allocation: {},
                        totalAllocated: 0,
                        packagingSpecSheetRef: defaultTemplate.packagingSpecSheetRef ?? null,
                    };
                    setBaseInstruction(template);
                    if (!isEditing) {
                        setCurrentInstruction(template);
                    }
                }
            })
            .catch((err) => console.error('Failed to load packing instructions', err));
    }, [isEditing]);
