...
export const EmbellishmentPlanGenerator: React.FC<EmbellishmentPlanGeneratorProps> = ({ job, onClose, onIssue }) => {
    ...
    const consolidatedItems: ConsolidatedEmb[] = useMemo(() => {
        ...
        job.styles.forEach(style => {
            let embellishments = style.embellishments;
            if (typeof embellishments === 'string') {
                try { embellishments = JSON.parse(embellishments); } catch (e) { embellishments = []; }
            } else if (!embellishments) {
                embellishments = [];
            }

            if (Array.isArray(embellishments)) {
                embellishments.forEach((emb: EmbellishmentRecord) => {
                    ...
                    if (emb.usageRule === 'Generic') {
                        ...
                        styleBreakdown.push({ label: 'All Units', styleNo: style.styleNo, buyer: typeof style.buyer === 'object' && style.buyer ? style.buyer.name : style.buyer || '', qty: style.quantity });
                        ...
                        style.sizeGroups?.forEach(sg => {
                            ...
                            if (groupQty > 0) {
                                ...
                                styleBreakdown.push({ label: `Group: ${sg.groupName}`, styleNo: style.styleNo, buyer: typeof style.buyer === 'object' && style.buyer ? style.buyer.name : style.buyer || '', qty: groupQty });
                            }
                        });
                        ...
                        Object.keys(emb.dimensionsData).forEach(sz => {
                            if (sizeQtys[sz]) {
                                ...
                                styleBreakdown.push({ label: `Size: ${sz}`, styleNo: style.styleNo, buyer: typeof style.buyer === 'object' && style.buyer ? style.buyer.name : style.buyer || '', qty: sizeQtys[sz] });
                            }
                        });
                    }
