                        ...
                        {job.styles.map((style) => (
                            <div key={style.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                                <div className="bg-gray-50 px-4 py-3 border-b border-gray-100 flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tight">
                                            {typeof style.buyer === 'object' && style.buyer !== null ? style.buyer.name : style.buyer}
                                        </p>
                                    </div>
                                ...
                                <div className="p-4 space-y-4">
                                    {(() => {
                                        let finishing = style.finishing;
                                        if (typeof finishing === 'string') {
                                            try { finishing = JSON.parse(finishing); } catch (e) { finishing = null; }
                                        }

                                        if (finishing?.packingList && finishing.packingList.length > 0) {
                                            return finishing.packingList.map((inst: any, idx: number) => (
