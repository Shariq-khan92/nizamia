...
import { Order, JobBatch, PurchasingRequest, CompanyDetails, IssuedPurchaseOrder, POLineItem, POItemVariant, MaterialReception, Supplier } from '../types';
...
interface PurchasingDashboardProps {
  ...
  onCreatePO?: (po: IssuedPurchaseOrder) => Promise<IssuedPurchaseOrder>;
  onUpdatePO?: (id: string, po: IssuedPurchaseOrder) => Promise<IssuedPurchaseOrder>;
...
export const PurchasingDashboard: React.FC<PurchasingDashboardProps> = ({
  ...
  onCreatePO, onUpdatePO, onUpdateJob, suppliers = []
}) => {
  ...
  const handleConfirmPO = () => {
    ...
    const confirmCreation = async () => {
      if (onCreatePO) {
        try {
          const saved = await onCreatePO(newPO);
          onUpdateIssuedPOs([saved, ...issuedPOs]);
        } catch (e) {
          console.error(e);
      ...
      if (onUpdateJobs) { // Still use onUpdateJobs for local state, but try to persist if onUpdateJob provided
        const affectedJobs: JobBatch[] = [];
        const updatedJobs = jobs.map(job => {
          ...
          const updatedJob = { ...job, purchasingRequests: updatedRequests };
          affectedJobs.push(updatedJob);
          return updatedJob;
        });
        ...
        if (onUpdateJob) {
          affectedJobs.forEach(async (j) => {
            try { await onUpdateJob(j.id, j); } catch (e) { console.error("Failed to update job status", e); }
          });
        }
      }
  ...
  const handleConfirmReception = () => {
    ...
    const processReception = async () => {
      ...
      updatedPOs = updatedPOs.map(po => {
        const isAffected = receivingItems.some(item => item.poId === po.id);
        if (!isAffected) return po;

        const newReceptionsForPO = receivingItems.filter(item => item.poId === po.id).map(item => {
          ...
          if (!isNaN(qtyReceived) && qtyReceived > 0) {
            ...
            } as MaterialReception;
          }
          return null;
        }).filter(Boolean) as MaterialReception[];

        if (newReceptionsForPO.length === 0) return po;

        affectedPOIds.add(po.id);
        const currentReceptions = po.receptions || [];
        const nextReceptions = [...currentReceptions, ...newReceptionsForPO];
      ...
      if (onUpdatePO) {
        for (const poId of affectedPOIds) {
          ...
          if (updatedPO) {
            try { await onUpdatePO(poId, updatedPO); } catch (e) { console.error(e); }
          }
        }
      }
      ...
      if (onUpdateJobs) {
        ...
        const updatedJobs = jobs.map(job => {
          ...
          const updatedJob = { ...job, purchasingRequests: updatedRequests };
          if (JSON.stringify(job) !== JSON.stringify(updatedJob)) affectedJobs.push(updatedJob);
          return updatedJob;
        });
        onUpdateJobs(updatedJobs);
        ...
        if (onUpdateJob) {
          affectedJobs.forEach(async (j) => {
            try { await onUpdateJob(j.id, j); } catch (e) { console.error(e); }
          });
        }
      }
  ...
  const executePrint = () => {
    ...
    const supplierInfo = suppliers.find(s => s.name === poToPrint.supplierName);
          ...
          <div className="bg-white w-full max-w-[90vw] max-h-[90vh] rounded-xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/30">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                <div className="space-y-1.5"><label className="text-xs font-black text-gray-500 uppercase tracking-widest">SUPPLIER</label><select value={draftPO.supplierName} onChange={(e) => setDraftPO({ ...draftPO, supplierName: e.target.value })} className="w-full px-3 py-2 bg-white border border-gray-300 rounded-md text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm">{suppliers.map(sup => (<option key={sup.id} value={sup.name}>{sup.name}</option>))}{!suppliers.find(s => s.name === draftPO.supplierName) && (<option value={draftPO.supplierName}>{draftPO.supplierName}</option>)}</select></div>
