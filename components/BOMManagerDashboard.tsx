...
interface BOMManagerDashboardProps {
   ...
   onAddItem?: (item: MasterBOMItem) => Promise<MasterBOMItem | void>;
   onUpdateItem?: (id: string, item: Partial<MasterBOMItem>) => Promise<MasterBOMItem | void>;
   onDeleteItem?: (id: string) => Promise<void>;
   bomPresets: BOMPreset[];
   setBomPresets: (presets: BOMPreset[]) => void;
   onAddPreset?: (preset: BOMPreset) => Promise<BOMPreset | void>;
   onUpdatePreset?: (id: string, preset: Partial<BOMPreset>) => Promise<BOMPreset | void>;
...
export const BOMManagerDashboard: React.FC<BOMManagerDashboardProps> = ({
   masterItems, setMasterItems, onAddItem, onUpdateItem, onDeleteItem, bomPresets, setBomPresets, onAddPreset, onUpdatePreset, onDeletePreset, buyers, suppliers
}) => {
   ...
   const handleSave = async () => {
      if (editingItem) {
         ...
         if (onUpdateItem) {
            try {
               const updated = await onUpdateItem(editingItem.id, formData);
               if (updated) {
                  setMasterItems(masterItems.map(i => i.id === editingItem.id ? updated : i));
               } else {
         ...
         if (onAddItem) {
            try {
               const saved = await onAddItem(newItem);
               if (saved) {
   ...
   const handleSavePreset = async () => {
      ...
      if (editingPreset) {
         ...
         if (onUpdatePreset) {
            try {
               const updated = await onUpdatePreset(editingPreset.id, presetForm);
               if (updated) {
                  setBomPresets(bomPresets.map(p => p.id === editingPreset.id ? updated : p));
               } else {
         ...
         // Create new preset
         const newPreset: BOMPreset = { ...presetForm, id: `pre-${Date.now()}` } as BOMPreset;
         if (onAddPreset) {
            try {
               const saved = await onAddPreset(newPreset);
               if (saved) {
                  setBomPresets([...bomPresets, saved]);
               } else {
                  setBomPresets([...bomPresets, newPreset]);
               }
            } catch (err) {
