...
import { PRODUCTION_TOOLS } from '../constants';
...
type GarmentTemplateStep = { op: string; machine: string };

interface ThreadConsumptionModalProps {
  onClose: () => void;
  operationsByCategory?: Record<string, string[]>;
  machineFactors?: Record<string, number>;
  garmentTemplates?: Record<string, GarmentTemplateStep[]>;
}

export const ThreadConsumptionModal = ({ onClose, operationsByCategory = THREAD_OPERATIONS, machineFactors = MACHINE_FACTORS, garmentTemplates = GARMENT_TEMPLATES }: ThreadConsumptionModalProps) => {
  const machineList = Object.keys(machineFactors).length > 0 ? Object.keys(machineFactors) : ['Lockstitch'];
  const [rows, setRows] = useState<{ id: string, operation: string, machine: string, length: string }[]>([
    { id: '1', operation: '', machine: machineList[0] || 'Lockstitch', length: '0' }
  ]);
  ...
  const loadTemplate = (templateName: string) => {
    const template = garmentTemplates[templateName];
    if (template) {
  ...
  const totals = useMemo(() => {
    ...
    rows.forEach(row => {
      ...
      const factor = machineFactors[row.machine] || 0;
      totalCm += len * factor;
    });
...
// Thread/machine/template data now fetched from API
              {Object.keys(garmentTemplates).map(tpl => (
                <button
                  key={tpl}
                  ...
                  return (
                    <tr key={row.id} className="group hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2">
                        ...
                        >
                          {Object.keys(machineFactors).map(m => (
...
export const PantoneConverterModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const [result, setResult] = useState('');
  const [extractedHex, setExtractedHex] = useState('');
  const [loading, setLoading] = useState(false);
  ...
  const handleSearch = async () => {
    ...
    const prompt = `You are a color conversion assistant. Given Pantone code ${pantoneCode}, provide approximate RGB values, HEX code, and recommended matching threads/zippers. Format results cleanly.`;
    ...
    try {
      ...
      if (hexMatch) {
        setExtractedHex(`#${hexMatch[1].toUpperCase()}`);
      }
    } catch (e) {
      setResult('Error fetching data. Please try again.');
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
          <h2 className="text-lg font-bold text-[#37352F] flex items-center gap-2">
            <Palette size={20} className="text-red-600" /> Pantone Color Converter
          </h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
        </div>

        <div className="p-6 space-y-6 overflow-y-auto">
          <div className="flex gap-2 p-2 bg-indigo-50 rounded-lg border border-indigo-100 items-start">
            <Sparkles size={16} className="text-indigo-600 mt-0.5 shrink-0" />
            <p className="text-xs text-indigo-700 leading-tight">
              Enter a Pantone code to get approximate RGB/HEX values and closest thread/zipper matches.
            </p>
          </div>

          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-500 uppercase">Pantone Code</label>
            <input
              type="text"
              value={pantoneCode}
              onChange={(e) => setPantoneCode(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded focus:border-red-500 outline-none"
              placeholder="e.g. 19-4052"
              autoFocus
            />
            <button
              onClick={handleSearch}
            ...
            >
              {loading ? (
                ...
                <>
                  <Search size={16} /> Convert Color
                </>
              )}
            </button>
          </div>

          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 min-h-[180px] relative">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Result</label>
            {loading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-50/80 rounded-lg gap-2">
...
export const ResourcesDashboard: React.FC<ResourcesDashboardProps> = ({ onOpenTool }) => {
  ...
  const [threadOpsByCategory, setThreadOpsByCategory] = useState<Record<string, string[]>>({});
  const [machineFactors, setMachineFactors] = useState<Record<string, number>>({});
  const [garmentTemplates, setGarmentTemplates] = useState<Record<string, { op: string; machine: string }[]>>({});

  // Load settings data from API
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const [ops, factors, templates] = await Promise.all([
          api.getThreadOperations(),
          api.getMachineFactors(),
          api.getGarmentTemplates(),
        ]);

        const groupedOps = (ops || []).reduce((acc: Record<string, string[]>, op: any) => {
          const cat = op.category || 'General';
          acc[cat] = acc[cat] || [];
          acc[cat].push(op.name);
          return acc;
        }, {});

        const factorMap = (factors || []).reduce((acc: Record<string, number>, f: any) => {
          acc[f.machineType] = Number(f.factor) || 0;
          return acc;
        }, {});

        const templateMap = (templates || []).reduce((acc: Record<string, { op: string; machine: string }[]>, t: any) => {
          const steps = (t.GarmentTemplateStep || [])
            .sort((a: any, b: any) => (a.stepOrder || 0) - (b.stepOrder || 0))
            .map((s: any) => ({ op: s.operation, machine: s.machineType }));
          acc[t.name] = steps;
          return acc;
        }, {});

        setThreadOpsByCategory(groupedOps);
        setMachineFactors(factorMap);
        setGarmentTemplates(templateMap);
      } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full space-y-6 pb-10">
      ...
      {activeCalculator === 'sewing-thread' && (
        <ThreadConsumptionModal
          onClose={() => setActiveCalculator(null)}
          operationsByCategory={threadOpsByCategory}
