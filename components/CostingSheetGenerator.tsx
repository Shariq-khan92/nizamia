...
const getInitialState = (): CostingState => ({
...
export const CostingSheetGenerator: React.FC<CostingSheetGeneratorProps> = ({ onBack, initialId }) => {
   const [data, setData] = useState<CostingState>(getInitialState);
   ...
   // Load existing data if initialId is provided
   useEffect(() => {
      if (initialId) {
         const loadCosting = async () => {
            try {
               const costing = await api.getCostingById(initialId);
               setDbId(costing.id);
               setData({
                  id: costing.costingId,
                  status: costing.status as any,
                  meta: costing.meta,
                  currency: costing.currency,
                  fabric: costing.fabric,
                  cm: costing.cm,
                  washing: costing.washing,
                  trims: costing.trims,
                  embellishment: costing.embellishment,
                  margins: costing.margins
               });
            } catch (error) {
   ...
   const handleSave = async () => {
      ...
      try {
         const payload = {
            ...data, // This includes meta, currency, arrays, margins
            finalFobPkr: totals.finalFobPkr,
            finalFobUsd: totals.finalFobUsd,
            marginPct: data.margins.profitPct // Using profitPct as marginPct for now
         };

         let savedCosting;
         if (dbId) {
            // Update existing
            savedCosting = await api.updateCosting(dbId, payload);
         } else {
            // Create new
            savedCosting = await api.createCosting(payload);
            setDbId(savedCosting.id);
         }

         setLastSaved(new Date().toLocaleTimeString());
         console.log("Saved Costing Sheet:", savedCosting);
      } catch (error) {
   ...
   const handleApprove = async () => {
      const updatedData = { ...data, status: 'Approved' as const };
      setData(updatedData);

      // Save immediately with new status
      setIsSaving(true);
      try {
         const payload = {
            ...updatedData,
            finalFobPkr: totals.finalFobPkr,
            finalFobUsd: totals.finalFobUsd,
            marginPct: data.margins.profitPct
         };

         if (dbId) {
            await api.updateCosting(dbId, payload);
         } else {
            const saved = await api.createCosting(payload);
            setDbId(saved.id);
         }
         setLastSaved(new Date().toLocaleTimeString());
      } catch (error) {
         console.error("Failed to approve costing:", error);
      } finally {
         setIsSaving(false);
      }
   };
   ...
   const handleRevise = async () => {
      // Logic for revision: Create a NEW copy with incremented ID/revision?
      // For now, simpler logic: duplicate the current sheet as a new DRAFT
      // But user might just want to edit status back to Draft?
      // Let's set status to Revised (which acts like Draft)

      const updatedData = { ...data, status: 'Revised' as const };
      setData(updatedData);
      ...
      try {
         ...
         console.error("Failed to revise costing:", error);
      } finally {
         setIsSaving(false);
      }
   };
